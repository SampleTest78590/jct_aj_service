...
@Component({
  selector: 'app-upload-lead-list-sms',
  templateUrl: './upload-lead-list-sms.component.html',
  styleUrls: ['./upload-lead-list-sms.component.scss'],
  encapsulation:ViewEncapsulation.None,
})
export class UploadLeadListSmsComponent implements OnInit {
  @Input() data: any = {}
  ...
  constructor(
    private uploadService: JctServicesService,private dialogRefSelf: DialogService,
     public toastservice: ToastService, private smsService: JctSmsService) { }
  ...
  downloadSample() {
    this.uploadService.downloadSampleFile().subscribe((result: any) => {
  ...
  Apivalidationcall() {
    ...
    this.subscriptonLst.push(
      this.smsService.validateleadlist(formData).subscribe((resp: any) => {
  ...
  UploadSubmit() {
    ...
    formData.append("leadDescription", this.leadDescription);
    ...
    this.subscriptonLst.push(
      this.smsService.smsleadlistupload(formData).subscribe((resp: any) => {
        if (resp.status == "Success") {
          this.toastservice.showToast({'title': resp?.message.message, 'kind': 'success'});
          this.dialogRefSelf.close(resp?.message);
        }
        else {
          this.toastservice.showToast({'title': resp?.message.message, 'kind': 'error'});
        }
      }, (err: any) => {
        this.toastservice.showToast({'title': err?.error.message, 'kind': 'error'});
      })
